...
def rot6d_to_axis_angle(rot6d):
    ...
    rot_mat_3x3 = torch.stack((b1, b2, b3), dim=-1)  # 3x3 rotation matrix
    ...
    axis_angle = kornia.geometry.conversions.rotation_matrix_to_axis_angle(rot_mat_3x3).reshape(-1, 3)  # axis-angle
...
def axis_angle_to_rot6d(angle_axis):
    ...
    rot_mat = kornia.geometry.conversions.axis_angle_to_rotation_matrix(angle_axis)  # 4x4 rotation matrix
    rot6d = rot_mat[:, :3, :2]
...
def axis_angle_to_quaternion(angle_axis):
    ...
    quaternion = kornia.geometry.conversions.axis_angle_to_quaternion(angle_axis)
    return quaternion
...
def mat3x3_to_axis_angle(rot_mat):
    ...
    #axis_angle = tgm.rotation_matrix_to_angle_axis(rot_mat).reshape(-1, 3)  # axis-angle
    axis_angle = kornia.geometry.conversions.rotation_matrix_to_axis_angle(rot_mat).reshape(-1, 3)  # axis-angle
    axis_angle[torch.isnan(axis_angle)] = 0.0
    return axis_angle
...
def flip_orientations(global_orient):
    ...
    rotation_matrices = kornia.geometry.conversions.axis_angle_to_rotation_matrix(global_orient)
    ...
    cos_angle, sin_angle = torch.cos(torch.tensor(np.pi)), torch.sin(torch.tensor(np.pi))
    y_rot_matrix_3x3 = torch.tensor([[cos_angle.item(), 0, sin_angle.item()],
                                     [0, 1, 0],
                                     [-sin_angle.item(), 0, cos_angle.item()]], device=global_orient.device)
    y_rot_matrix_batch_3x3 = y_rot_matrix_3x3.unsqueeze(0).repeat(global_orient.size(0), 1, 1)
    flipped_matrices = torch.matmul(rotation_matrices, y_rot_matrix_batch_3x3) # 3x3同士の積

    # Convert rotation matrices back to rotation vectors
    #flipped_orient = tgm.rotation_matrix_to_angle_axis(flipped_matrices)
    #flipped_orient = kornia.geometry.conversions.rotation_matrix_to_angle_axis(flipped_matrices)

    # Return the adjusted rotation vectors
    #return flipped_orient[:, :3]
    return kornia.geometry.conversions.rotation_matrix_to_axis_angle(flipped_matrices)
...
def correct_orientation(global_orient):
    ...
    #                                                                                              1, 1)
    #cos_angle, sin_angle = torch.cos(torch.tensor(np.pi)), torch.sin(torch.tensor(np.pi))
    cos_angle = torch.cos(torch.tensor(np.pi))
    sin_angle = torch.sin(torch.tensor(np.pi))
    y_rot_matrix_3x3 = torch.tensor([
        [cos_angle.item(), 0, sin_angle.item()],
        [0, 1, 0],
        [-sin_angle.item(), 0, cos_angle.item()]
    ...
    #                              [0, 0, 0, 1]], device=global_orient.device).unsqueeze(0).repeat(global_orient.size(0),
    #                                                                                              1, 1)
    x_rot_matrix_3x3 = torch.tensor([
        [1, 0, 0],
        [0, cos_angle.item(), -sin_angle.item()],
        [0, sin_angle.item(), cos_angle.item()]
    ], device=global_orient.device)
    ...
    #                              [0, 0, 0, 1]], device=global_orient.device).unsqueeze(0).repeat(global_orient.size(0),
    #                                                                                              1, 1)
    z_rot_matrix_3x3 = torch.tensor([
        [cos_angle.item(), -sin_angle.item(), 0],
        [sin_angle.item(), cos_angle.item(), 0],
        [0, 0, 1]
    ], device=global_orient.device)

    y_rot_matrix_batch_3x3 = y_rot_matrix_3x3.unsqueeze(0).repeat(global_orient.size(0), 1, 1)
    x_rot_matrix_batch_3x3 = x_rot_matrix_3x3.unsqueeze(0).repeat(global_orient.size(0), 1, 1)
    z_rot_matrix_batch_3x3 = z_rot_matrix_3x3.unsqueeze(0).repeat(global_orient.size(0), 1, 1)

    # Apply Y rotation to each rotation matrix
    #combined_rot_matrix = torch.matmul(y_rot_matrix, z_rot_matrix)
    combined_rot_matrix_3x3 = torch.matmul(y_rot_matrix_batch_3x3, z_rot_matrix_batch_3x3)
    #print(combined_rot_matrix)
    print(combined_rot_matrix_3x3)
    #flipped_matrices = torch.matmul(rotation_matrices, combined_rot_matrix)[:, :3, :]
    flipped_matrices = torch.matmul(rotation_matrices, combined_rot_matrix_3x3)
    # flipped_matrices = torch.matmul(combined_rot_matrix, y_rot_matrix)[:, :3, :]

    # Convert rotation matrices back to rotation vectors
    #flipped_orient = tgm.rotation_matrix_to_angle_axis(flipped_matrices)
    flipped_orient = kornia.geometry.conversions.rotation_matrix_to_axis_angle(flipped_matrices)
